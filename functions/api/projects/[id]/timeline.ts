// functions/api/projects/[id]/timeline.ts
// POST /api/projects/:id/timeline - append a timeline note (member ok).
...
export const onRequestPost: PagesFunction = async ({ request, env, params, data }) => {
  ...
  try {
    ...
    const row = await env.DB.prepare(
      `INSERT INTO timeline (project_id, when_label, note, created_by)
       VALUES (?, ?, ?, ?)
       RETURNING *`
    )
    ...
    console.error("POST /api/projects/:id/timeline failed:", e);
    return error("failed to add timeline note", 500);
  }
};
